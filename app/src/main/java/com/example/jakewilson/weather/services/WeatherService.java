...
import com.example.jakewilson.weather.util.UnitLocale;
...
public class WeatherService {
    ...
    public void onLoadWeather(LoadWeatherEvent event){
        mApi.getForecast(event.getLat(), event.getLon(), UnitLocale.imperialOrMetric(Locale.getDefault())).enqueue(new Callback<WeatherResponse>() {
            @Override
            public void onResponse(Response<WeatherResponse> response) {
                ...
                mBus.post(new WeatherLoadedEvent(response.body()));
            }
